...
{ 
    ...
    { 
        ...
        { 
            var cam = await _context.Cameras.Include(c => c.CameraAlgorithms).FirstOrDefaultAsync(c => c.Id == id); 
            ...
            Input = new CameraInput { 
                ...
                ConnectionMethodId = cam.ConnectionMethodId, 
                AreaId = cam.AreaId, 
                GroupId = cam.GroupId, 
                AlgorithmIds = cam.CameraAlgorithms.Select(x => x.AlgorithmId).ToList() 
            }; 
        ...
        { 
            ...
            var cam = await _context.Cameras.Include(c => c.CameraAlgorithms).FirstOrDefaultAsync(c => c.Id == Input.Id); 
            if(cam == null) return NotFound(); 
            
            cam.Name = Input.Name; 
            cam.DeviceId = Input.DeviceId; 
            cam.CameraId = Input.CameraId; 
            cam.StreamUrl = Input.StreamUrl; 
            cam.ConnectionMethodId = Input.ConnectionMethodId; 
            cam.AreaId = Input.AreaId; 
            cam.GroupId = Input.GroupId; 
            cam.CameraAlgorithms.Clear(); 
            
            if(Input.AlgorithmIds != null) 
            { 
                foreach(var id in Input.AlgorithmIds) 
                {
                    cam.CameraAlgorithms.Add(new CameraAlgorithm { CameraId = cam.Id, AlgorithmId = id }); 
                }
            } 
